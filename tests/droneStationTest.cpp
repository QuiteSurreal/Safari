...
TEST(DroneStationIntegrationTest, RotateFacesCameraAndFlipsWhenDestinationIsBehind) {
    using namespace std;

    Vec3 spawnPos(0, 0, 0);
    Entity* objEntity = &Factory::createDroneStation(spawnPos);
    ASSERT_NE(objEntity, nullptr);
    ASSERT_TRUE(objEntity->hasComponent<DroneStation>());
    ASSERT_TRUE(objEntity->hasComponent<Transform>());

    DroneStation& ent = objEntity->getComponent<DroneStation>();
