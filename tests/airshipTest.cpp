...
TEST(AirshipIntegrationTest, RotateFacesCameraAndFlipsWhenDestinationIsBehind) {
    using namespace std;

    Vec3 spawnPos(0, 0, 0);
    Entity* objEntity = &Factory::createAirship(spawnPos);
    ASSERT_NE(objEntity, nullptr);
    ASSERT_TRUE(objEntity->hasComponent<Airship>());
    ASSERT_TRUE(objEntity->hasComponent<Transform>());

    Airship& ent = objEntity->getComponent<Airship>();
