...
WildebeestHerd::WildebeestHerd() : Behaviour() {
...
// TEMP
void WildebeestHerd::updateUniqueRoaming(Entity& ent) {
	ent.getComponent<Wildebeest>().setIdleCounter(0);
}

void WildebeestHerd::updateState() {

	// TODO based on innter state

}

bool WildebeestHerd::areAnimalsSatisfied() {
	return false; // TODO
}

void WildebeestHerd::assessThreat() {} // TODO 

void WildebeestHerd::assessNeighbors() {} // TODO

void WildebeestHerd::calculateMigratePositions() {
...
void WildebeestHerd::assignUniqueHerdId(int id) {

	globalHerdId = id;
	herdLabel = "wildebeestHerd:" + std::to_string(globalHerdId);

}

const std::tuple<int, int, std::string> WildebeestHerd::addAnimal() {
...
void WildebeestHerd::removeAnimalFromHerd(int localId) {

	std::string animalLabel = herdLabel + ":" + std::to_string(localId);
	Entity& ent = Labels::getUnique(animalLabel);
	ent.getComponent<Wildebeest>().setHerdId(-1);
	ent.getComponent<Labels>().removeLabel(animalLabel);

}

std::vector<std::string> WildebeestHerd::getAnimalIds() {

	std::vector<std::string> ids;
	for (int i = 0; i < herdSize; ++i) {
		ids.push_back("wildebeestHerd:" + std::to_string(globalHerdId) + ":" + std::to_string(i));
	}
...
void WildebeestHerd::start(Entity& entity) { }

void WildebeestHerd::update(Entity& entity) {
...
void WildebeestHerd::updateAnimalRoaming() {

	for (int i = 0; i < herdSize; i++) {
		Entity& ent = Labels::getUnique(herdLabel + ":" + std::to_string(i));
		if (ent.getComponent<Wildebeest>().getIdleCounter() <= 0 && ent.getComponent<Wildebeest>().getState() == "Neutral") {
			...
			ent.getComponent<Wildebeest>().setIdleCounter(ent.getComponent<Wildebeest>().getIdleMax());
		}
	}
...
Vec3 WildebeestHerd::getRandomAnimalRoamingPosition() {
...
void WildebeestHerd::updateAnimalMigration(Entity& entity) {
	...
	for (int i = 0; i < herdSize; i++) {
		...
		// Can be optimized
		Entity& ent = Labels::getUnique(herdLabel + ":" + std::to_string(i));
		if (ent.getComponent<Wildebeest>().getState() == "Neutral") {
