...
GiraffeHerd::GiraffeHerd() : Behaviour() {
...
// TEMP
void GiraffeHerd::updateUniqueRoaming(Entity& ent) {
	ent.getComponent<Giraffe>().setIdleCounter(0);
}

void GiraffeHerd::updateState() {

	// TODO based on innter state

}

bool GiraffeHerd::areAnimalsSatisfied() {
	return false; // TODO
}

void GiraffeHerd::assessThreat() {} // TODO 

void GiraffeHerd::assessNeighbors() {} // TODO

void GiraffeHerd::calculateMigratePositions() {
...
void GiraffeHerd::assignUniqueHerdId(int id) {

	globalHerdId = id;
	herdLabel = "giraffeHerd:" + std::to_string(globalHerdId);

}

const std::tuple<int, int, std::string> GiraffeHerd::addAnimal() {
...
void GiraffeHerd::removeAnimalFromHerd(int localId) {

	std::string animalLabel = herdLabel + ":" + std::to_string(localId);
	Entity& ent = Labels::getUnique(animalLabel);
	ent.getComponent<Giraffe>().setHerdId(-1);
	ent.getComponent<Labels>().removeLabel(animalLabel);

}

std::vector<std::string> GiraffeHerd::getAnimalIds() {

	std::vector<std::string> ids;
	for (int i = 0; i < herdSize; ++i) {
		ids.push_back("giraffeHerd:" + std::to_string(globalHerdId) + ":" + std::to_string(i));
	}
...
void GiraffeHerd::start(Entity& entity) { }

void GiraffeHerd::update(Entity& entity) {
...
void GiraffeHerd::updateAnimalRoaming() {

	for (int i = 0; i < herdSize; i++) {
		Entity& ent = Labels::getUnique(herdLabel + ":" + std::to_string(i));
		if (ent.getComponent<Giraffe>().getIdleCounter() <= 0 && ent.getComponent<Giraffe>().getState() == "Neutral") {
			...
			ent.getComponent<Giraffe>().setIdleCounter(ent.getComponent<Giraffe>().getIdleMax());
		}
	}
...
Vec3 GiraffeHerd::getRandomAnimalRoamingPosition() {
...
void GiraffeHerd::updateAnimalMigration(Entity& entity) {
	...
	for (int i = 0; i < herdSize; i++) {
		...
		// Can be optimized
		Entity& ent = Labels::getUnique(herdLabel + ":" + std::to_string(i));
		if (ent.getComponent<Giraffe>().getState() == "Neutral") {
