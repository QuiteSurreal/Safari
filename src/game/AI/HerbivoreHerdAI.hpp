...
class HerbivoreHerdAI : public Behaviour, public IHerdRelationshipInteraction {
...
public:
    ...
    /// <summary>
    /// Adds a new species relationship to the herd's registry.
    /// </summary>
    /// <param name="speciesLabel">The fellow species' label.</param>
    /// <param name="modifier">The mood-modifier.</param>
    void addSpeciesRelationship(const std::string& speciesLabel, float modifier);

    /// <summary>
    /// Removes a species relationship from the herd's registry.
    /// </summary>
    /// <param name="speciesLabel">The fellow species' label.</param>
    void removeSpeciesRelationship(const std::string& speciesLabel);

    /// <summary>
    /// Picks a valid food source from the current site.
    /// </summary>
    /// <returns>If a valid food source was found, it is returned. Null pointer otherwise.</returns>
    Entity* pickValidFoodSource();

    /// <summary>
    /// Picks a valid water source from the current site.
    /// </summary>
    ...
    int getIntersectingAnimals(Vec3 otherHerdCenter, float otherHerdTerritoryRadius);
    ...
    float getNeighborEffect() const { return neighborEffect; }
    void setNeighborEffect(float value) { neighborEffect = value; }
...
private:
    ...
    /// <summary>
    /// Assesses the herd's threat level by checking the animal's spook levels.
    /// </summary>
    void assessThreat();

    /// <summary>
    /// Assesses the herd's overall attitude towards their current neighbours.
    /// </summary>
    ...
    /// <summary>
    /// Notifies animals that the herd is at a site or not.
    /// </summary>
    /// <param name="atSite">Boolean value denoting whether the herd is stationing at a site.</param>
    void signalAtSiteToAnimals(bool atSite);
    ...
    /// </summary>
    void enterHeadingToPreferredState();
