...
bool HerbivoreAI::askForFood() {

    Entity* foodSource = Labels::getUnique(herdLabel).getComponent<HerbivoreHerdAI>().pickValidFoodSource();

    if (!foodSource)
        return false;

    if (foodSource->getComponent<Targetable>().target()) {
...
bool HerbivoreAI::askForWater() {

    std::optional<Vec3> waterSource = Labels::getUnique(herdLabel).getComponent<HerbivoreHerdAI>().pickValidWaterSource();
...
float HerbivoreAI::calculateFoodPercentage() {
    return (foodMax == 0) ? 0.0f : static_cast<float>(food) / foodMax;
}

float HerbivoreAI::calculateWaterPercentage() {
    return (waterMax == 0) ? 0.0f : static_cast<float>(water) / waterMax;
}


float HerbivoreAI::calculateSpookPercentage() {
    return (spookMax == 0) ? 0.0f : static_cast<float>(spook) / spookMax;
}


void HerbivoreAI::directSpook(Vec3 source, int spookLevel) {
    sourceOfFear = source;
    spook = spookLevel;
    if (spook > 0) {
        enterSpookedState();
...
void HerbivoreAI::addSourceOfFear(const std::string label, float fearRange) {
    fearSourceLabels[label] = fearRange;
}


void HerbivoreAI::removeSourceOfFear(const std::string label) {
    fearSourceLabels.erase(label);
}


void HerbivoreAI::breed(Entity* other) {

    if (other->getComponent<Targetable>().target()) {

        breedingPartner = other;
        breedingPartner->getComponent<HerbivoreAI>().enterHornyState();
...
void HerbivoreAI::start(Entity& entity) {
...
void HerbivoreAI::update(Entity& entity) {
    ...
    while (accumulator >= timeStep) {
        ...
        case State::Neutral: {
            ...
            if (!atSite) {
                ...
                // Water is preferred over food. No specific reason.
                if (waterPercent < 0.9f) {  // MAGIC NUMBER
                    enterEnRouteForWaterState();
                }
                else if (foodPercent < 0.9f) { // MAGIC NUMBER
...
void HerbivoreAI::updateAnimalLabel() {
    animalLabel = herdLabel + ":" + std::to_string(localId);
}


void HerbivoreAI::alertOtherAnimals() {

    Entity& herd = Labels::getUnique(herdLabel);
    std::vector<std::string> animals = herd.getComponent<HerbivoreHerdAI>().getAnimalIds();
    ...
    for (const std::string& id : animals) {
        ...
        if (inRange) {
            if (animal.getComponent<HerbivoreAI>().getState() != "Spooked") {
                animal.getComponent<HerbivoreAI>().directSpook(sourceOfFear, spook);
            }
        }
...
bool HerbivoreAI::updateEating() {

    // Check if target is still valid
    if (!currentFoodSource->getComponent<Targetable>().canTarget()) {
        currentFoodSource->getComponent<Targetable>().removeTargeting();
        return false;
    }

    // Check if the plant is still consumable.
    if (!currentFoodSource->getComponent<EdiblePlant>().isConsumable()) {
        return false;
    }
    ...
    if (glm::distance(getEntity().getComponent<Transform>().getPosition(), currentFoodSource->getComponent<Transform>().getPosition()) < 1.02f) {
        food += currentFoodSource->getComponent<EdiblePlant>().consume(20);
    }
...
bool HerbivoreAI::updateDrinking() {
...
void HerbivoreAI::killIfStarving() {
...
void HerbivoreAI::updateSpeed() {
    entity->getComponent<DestinationBasedMovement>().setSpeed(baseSpeed * speedModifier);
}

void HerbivoreAI::enterNeutralState() {
    state = State::Neutral;
    speedModifier = 1.0f;
    updateSpeed();
    Labels::getUnique(herdLabel).getComponent<HerbivoreHerdAI>().updateUniqueRoaming(*entity);
}

void HerbivoreAI::enterEnRouteForFoodState() {
...
void HerbivoreAI::enterEnRouteForWaterState() {
...
void HerbivoreAI::enterSpookedState() {
    state = State::Spooked;
...
void HerbivoreAI::enterHornyState() {
...
void HerbivoreAI::die()
{

    Entity herd = Labels::getUnique(herdLabel);
    herd.getComponent<HerbivoreHerdAI>().removeAnimalFromHerd(localId);
...
void HerbivoreAI::performBreeding() {
    ...
    if (breedingPartner->getComponent<HerbivoreAI>().getState() != "Horny") {
        abortBreeding();
        return;
    }
    ...
    if (breedingCounter >= 0) {
        ...
        breedingPartner->getComponent<HerbivoreAI>().bearNewAnimal();
        abortBreeding();
    }
...
void HerbivoreAI::bearNewAnimal() {

    Entity& baby = Labels::getUnique("eco").getComponent<HerdManager>().addAnimalToHerd(Labels::getUnique(herdLabel).getComponent<HerbivoreHerdAI>().getType(), sex, herdId);
    baby.getComponent<HerbivoreAI>().setAge(0);
    baby.getComponent<Transform>().relocate(getEntity().getComponent<Transform>().getPosition());
}


void HerbivoreAI::createLoveMarker() {
...
void HerbivoreAI::hideLoveMarker() {
    if (loveMarker)
        loveMarker->getComponent<Sprite>().setOpacity(1);
}


void HerbivoreAI::removeLoveMarker() {
    loveMarker->destroy();
    loveMarker = nullptr;
}


void HerbivoreAI::abortBreeding() {
    ...
    // For the partner.
    if (breedingPartner->getComponent<Targetable>().canTarget()) {
        //breedingPartner->getComponent<HerbivoreAI>().hideLoveMarker();
        breedingPartner->getComponent<HerbivoreAI>().removeLoveMarker();
        breedingPartner->getComponent<HerbivoreAI>().enterNeutralState();
        breedingPartner->getComponent<Targetable>().removeTargeting();
    }
    ...
    // Notifying the herd.
    HerbivoreHerdAI& ai = Labels::getUnique(herdLabel).getComponent<HerbivoreHerdAI>();
